...
import { CheckCircle, ArrowDown, Instagram, Mail, Globe, RotateCcw, Clock, AlertTriangle, Play, ChevronRight, ArrowRight } from 'lucide-react';
...
const StoryBlock = ({ title, children, align = "left", className = "" }) => {
  ...
  return (
    <div className={`py-16 px-6 md:px-20 max-w-7xl mx-auto ${className}`}>
      <FadeIn className={`max-w-4xl ${alignmentClasses[align]}`}>
        ...
        <div className="text-xl md:text-3xl font-serif leading-relaxed text-slate-800 space-y-6">
          {children}
        </div>
      </FadeIn>
    </div>
...
const MeetTony = () => {
  ...
  return (
    <section className="py-20 px-6 md:px-12 bg-slate-100 border-b border-slate-300">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <FadeIn>
          <div className="relative group flex justify-center">
            ...
            <img 
              src="/tony.png" 
              alt="Tony Low" 
              className="relative w-full max-w-sm object-contain drop-shadow-2xl transform hover:scale-105 transition duration-700" 
            />
          </div>
        </FadeIn>
        ...
        {/* TEXT SIDE */}
        <FadeIn delay={200}>
          <h2 className="text-5xl font-black mb-6 tracking-tighter text-slate-900">MEET TONY</h2>
          <div className="space-y-4 text-lg text-slate-700 font-light leading-relaxed">
            <p>
            ...
            <p className="font-serif italic text-xl text-slate-900 border-l-4 border-orange-500 pl-6 my-6">
          ...
          {gameState === 'prep' && (
            <div className="h-full flex flex-col p-8 items-center">
                 ...
                 {[...Array(targetShrimpCount)].map((_, i) => (
                   <div key={i} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handlePrepDrop(e, i)}
                     className={`w-28 h-28 border-2 border-slate-800 bg-white flex items-center justify-center transition-all relative rounded-lg
                       ${i === shrimpPrepped ? 'shadow-lg scale-105 border-green-500' : 'opacity-50'}
                     `}>
                     {i < shrimpPrepped ? (
              ...
              <div className="mt-auto">
                 <div draggable onDragStart={(e) => handleDragStart(e, 'knife')} className="bg-white border-2 border-slate-800 p-6 cursor-grab active:cursor-grabbing hover:-translate-y-1 transition-transform shadow-lg rounded-full animate-bounce">
                  ...
                  {requiredSauceIngredients.map((item) => (
                    <div key={item} draggable={!sauceIngredientsAdded.includes(item)} onDragStart={(e) => handleDragStart(e, item)}
                      className={`w-32 h-32 border-2 border-slate-800 bg-white flex flex-col items-center justify-center cursor-grab transition-all rounded-lg
                        ${sauceIngredientsAdded.includes(item) ? 'opacity-20 pointer-events-none' : 'hover:shadow-lg hover:scale-105 hover:-translate-y-1'}`}>
            ...
            <div className="h-full flex flex-col p-8 items-center relative">
              ...
              <div className="relative flex-1 w-full flex items-center justify-center">
                 <div onDragOver={(e) => e.preventDefault()} onDrop={handleWokDrop} className="w-[500px] h-[400px] flex items-center justify-center relative transition-transform">
                    {/* WOK TARGET INDICATOR */}
                    <div className="absolute inset-0 border-4 border-dashed border-orange-300 rounded-full animate-pulse opacity-50"></div>
                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-orange-600 text-white px-3 py-1 font-bold text-xs rounded animate-bounce">
                        DRAG HERE
                    </div>
              ...
              <div className="h-32 w-full border-t-2 border-slate-800 flex items-center justify-center gap-8 bg-gray-100 z-30">
                {cookingStep === 'empty' && (
                  <div draggable onDragStart={(e) => handleDragStart(e, 'prepped_shrimp')} className="cursor-move hover:-translate-y-1 transition text-center group bg-white border-2 border-slate-800 p-2 shadow-lg rounded-lg animate-bounce">
                ...
                {cookingStep === 'shrimp_cooked' && (
                   <div draggable onDragStart={(e) => handleDragStart(e, 'mixed_sauce')} className="cursor-move hover:-translate-y-1 transition text-center group bg-white border-2 border-slate-800 p-2 shadow-lg rounded-lg animate-bounce">
                 ...
                 {cookingStep === 'sauce_added' && (
                   <div draggable onDragStart={(e) => handleDragStart(e, 'egg')} className="cursor-move hover:-translate-y-1 transition text-center group bg-white border-2 border-slate-800 p-2 shadow-lg rounded-lg animate-bounce">
      ...
      {gameState === 'final' && (
        <div className="flex-1 flex flex-col items-center justify-center bg-white p-8">
          <div className="border-4 border-slate-800 p-12 max-w-2xl text-center shadow-2xl bg-stone-50 rounded-lg">
            ...
            <button onClick={startGame} className="bg-slate-900 text-white px-8 py-4 font-bold hover:bg-orange-600 transition-colors w-full flex items-center justify-center gap-2 rounded-lg"><RotateCcw size={20}/> RESTART SHIFT</button>
          </div>
        </div>
      )}
      ...
      {gameState === 'lost' && (
        <div className="flex-1 flex flex-col items-center justify-center bg-red-600 p-8 text-white">
          <div className="border-4 border-white p-12 max-w-2xl text-center rounded-lg">
            ...
            <button onClick={startGame} className="bg-white text-red-600 px-8 py-4 font-bold hover:bg-black hover:text-white transition-colors w-full flex items-center justify-center gap-2 tracking-wide uppercase rounded-lg"><RotateCcw size={20}/> TRY AGAIN</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-slate-900 selection:bg-orange-200 selection:text-orange-900 font-sans">
      ...
      >
        ...
        <div className="absolute bottom-10 left-6 md:left-12 relative z-10">
          <h1 className="text-[10vw] leading-[0.85] font-black tracking-tighter uppercase mix-blend-difference text-black">
            TONY'S<br/>KITCHEN LAB
          </h1>
        </div>

        <div className="absolute bottom-12 right-6 md:right-12 relative z-10">
          <ArrowDown className="animate-bounce w-10 h-10 text-slate-900" />
        </div>
      </header>
      ...
      <StoryBlock title="01. The Arrival">
        ...
        <p className="text-xl text-slate-600 font-sans mt-8 border-l-4 border-slate-300 pl-6">
      ...
      <StoryBlock title="02. The Recipe" align="right">
        <p>His dad brought home the recipe for <span className="underline decoration-4 underline-offset-4 decoration-orange-400">Shrimp with Lobster Sauce</span> from the restaurant where he worked.</p>
        ...
        <p className="italic text-slate-500 font-bold text-2xl">"Hey Jack," they'd call out at the fish markets, giving him the best pick.</p>
      </StoryBlock>

      {/* 5. THE FILM COMPONENT */}
      <div className="py-12 my-12 bg-slate-900 text-white">
      ...
      <StoryBlock title="03. Affection" align="center">
        <p>In the Low house, meals weren't conversational. You sat down, ate what was in front of you, cleaned your plate. <span className="font-bold text-slate-900 text-4xl block mt-4">That's how his mother showed love.</span></p>
        <p className="text-xl text-slate-500 mt-6">She'd stand at the stove for hours with one knife and a cleaver. The best piece of fish always went to the kids, her bowl was always smallest, and "I already ate" was her most reliable lie.</p>
      </StoryBlock>
      ...
      <StoryBlock title="04. Memory">
        ...
        <p className="text-orange-700 font-bold border-l-4 border-orange-500 pl-6">For the Shrimp with Lobster Sauce, she'd make this compote with fermented black beans, ginger, and garlic that had such a distinct, fascinating aroma. When she'd crack an egg into the wok, Tony knew the dish was minutes away.</p>
      </StoryBlock>

      {/* 8. ARCHIVE COMPONENT (MIXED MEDIA GRID) */}
      <div className="py-20 bg-slate-100 my-12">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1">
              ...
              <div className="relative aspect-[3/4] bg-orange-600 flex items-center justify-center p-8 text-center">
                  <h3 className="text-4xl font-black text-white leading-none">NO<br/>RECIPES<br/>JUST<br/>FEEL</h3>
              </div>
              {/* CELL 3: IMAGE */}
              <div className="relative aspect-[3/4] bg-white group overflow-hidden">
              ...
              {/* CELL 4: TEXT ART */}
              <div className="relative aspect-[3/4] bg-slate-900 flex items-center justify-center p-8 text-center">
                  <h3 className="text-4xl font-black text-white leading-none">ONE<br/>KNIFE<br/>ONE<br/>WOK</h3>
              </div>
              {/* CELL 5: IMAGE */}
              <div className="relative aspect-[3/4] bg-white group overflow-hidden">
      ...
      <div id="game-section" className="py-24 bg-orange-50 border-y border-orange-200 my-12">
        <div className="max-w-6xl mx-auto px-6">
      ...
      <StoryBlock title="07. Love Language" align="center">
        <p className="text-4xl md:text-7xl font-black text-slate-900">Tony's love language is food.</p>
        <p className="text-2xl text-slate-500 mt-8">You feed him, you're expressing your love. He cooks for you, he's giving it back.</p>
      </StoryBlock>

      {/* 13. ENDING QUESTIONS (IMPRESSIONABLE) */}
      <div className="bg-slate-900 text-white py-32 px-6 md:px-20 text-center">
        <FadeIn>
          <p className="font-mono text-xs uppercase tracking-widest text-orange-500 mb-16">WHAT MATTERS</p>
          <div className="space-y-16 max-w-5xl mx-auto">
            <p className="text-3xl md:text-5xl font-black leading-tight hover:text-orange-500 transition-colors cursor-default">
            ...
            <div className="h-[2px] bg-white/20 w-32 mx-auto"></div>
